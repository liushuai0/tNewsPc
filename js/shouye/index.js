$(function() {
	...
	$.ajax({
		type: "post", //请求方式
		url: "http://admin.jrjl.net/api/home/pc/leaders",
		dataType: "json",
		data: {
		...
		success: function(res) {
			...
			$.each(leaderlist, function(index) {
				$('#leaderList').append("<div class='d-flex flex-column align-items-center' onclick='goLeaderList(" +
					leaderlist[index].id + ")'><img src='" + leaderlist[index].thumbnail +
					"' style='width: 46px;height: auto;'><span style='font-size:14px'>" + leaderlist[index].title +
					"</span></div>")
			})
		},
	})
	...
	// });
	$("#footerguide> .shengjibumen").hover(function(){
		var fh = $(this).find('.addUl_list').innerHeight()
		
		$(this).find(".addUl_list").show()
		console.log(fh);
		
		$("#footerguide").stop().animate({"height":fh+50},500);
		console.log("jin");
	},function(){
		$(".addUl_list").css({'display':'none'})
		$("#footerguide").stop().animate({"height":"41px"},500);
		console.log('chu');
	})
	// $('#shengji').on('click', function() {
	// 	$("#addrUl").toggleClass('d-none')
	// 	$("#addrUl1").addClass('d-none')
	// 	$("#addrUl2").addClass('d-none')
	// });
	// $('#difang').on('click', function() {
	// 	$("#addrUl").addClass('d-none')
	// 	$("#addrUl1").toggleClass('d-none')
	// 	$("#addrUl2").addClass('d-none')
	// });
	// $('#youqing').on('click', function() {
	// 	$("#addrUl").addClass('d-none')
	// 	$("#addrUl1").addClass('d-none')
...
function showabc() {
	$.ajax({
		type: "get", //请求方式
		url: "http://admin.jrjl.net/api/home/pc/index",
		dataType: "json",
		data: {
		...
		success: function(res) {
			...
			if (HomeList.bannerlist != null) {
				$.each(HomeList.bannerlist, function(index) {
					if (index == 0) {
						$("#shoyeSwiper").append(
							"<div class='carousel-item position-relative active'><img class='d-block w-100' onclick='yinzhanToDetail(" +
							res.data.bannerlist[index].id + ")' style='height: 389px;cursor:pointer;' src='" + res.data.bannerlist[
								0].thumbnail + "' alt='Second slide'><div class='swiperText'><a href='newsDetail.html?id=" + res.data.bannerlist[
								index].id + "'>" + res.data.bannerlist[0].title + "</a></div></div>")
					} else {
						$("#shoyeSwiper").append(
							"<div class='carousel-item position-relative'><img class='d-block w-100' onclick='yinzhanToDetail(" + res.data
							.bannerlist[index].id + ")' style='height: 389px;' src='" +
							res.data.bannerlist[index].thumbnail +
							"' alt='Second slide'><div class='swiperText'><a href='newsDetail.html?id=" + res.data.bannerlist[index].id +
							"'>" + res.data.bannerlist[index].title + "</a></div></div>")
					}
				})
			...
			if (HomeList.toutiaolist.data != null) {
				...
				$("#toutiaoTitle").append("<a style='color: rgba(194, 27, 31, 1);' href='newsDetail.html?id=" + HomeList.toutiaolist
					.data.id + "'>" + HomeList.toutiaolist.data.title + "</a>")
				$("#toutiaoTitleM").attr("onclick", "leibiaoToDetail(" + HomeList.toutiaolist.cate_id + ")")
				$("#toutiaoTitleJJ").append("<a href='newsDetail.html?id=" + HomeList.toutiaolist.data.id + "'>" + HomeList.toutiaolist
					.data.description + "</a>")
			}
			...
			if (HomeList.kuaixunlist.data != null) {
				$.each(HomeList.kuaixunlist.data, function(index, value) {
					$("#kuaixunList").append("<span style='margin-left:10px'><a style='color:rgba(102, 102, 102, 1);font-size:14px' href='newsDetail.html?id=" + HomeList.kuaixunlist.data[index].id + "'>" +
						HomeList.kuaixunlist.data[index].title + "</a></span>")
				})
				// $.each(HomeList.kuaixunlist.data, function(index, value) {
			...
			if (HomeList.jiandulist.data != null) {
				...
				$("#jinriList1T").append("<a style='color: #333333;' href='newsDetail.html?id=" + HomeList.jiandulist.data[0].id +
					"'>" + HomeList.jiandulist.data[0].title + "</a>") //第一个标题 1Title
				...
				for (let index = 1; index < HomeList.jiandulist.data.length; index++) {
					$("#jinriList").append("<div><span class='dian'>·</span><a href='newsDetail.html?id=" + HomeList.jiandulist.data[
						index].id + "'>" + HomeList.jiandulist.data[index].title + "</a></div>")
				}
			}
			...
			if (HomeList.renshilist.data != null) {
				...
				$("#jujiaoTitleM").attr("onclick", "leibiaoToDetail(" + HomeList.renshilist.cate_id + ")")
				$("#jujiaoImg").attr("src", HomeList.renshilist.data[0].thumbnail)
				$("#jujiaoList1T").append("<a style='color: #C21B1F;' href='newsDetail.html?id=" + HomeList.renshilist.data[0].id +
					"'>" + HomeList.renshilist.data[0].title + "</a>") //第一个标题 1Title
				for (let index = 1; index < HomeList.renshilist.data.length; index++) {
					$("#jujiaoList").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" + HomeList.renshilist
						.data[index].id + "'>" + HomeList.renshilist.data[index].title + "</a></div>")
				}
			}
			//要闻处理  HomeList[4]
			if (HomeList.yaowenlist.data != null) {
				...
				//第一个card
				$("#yanwenContent1T").append(
					"<a style='color: rgba(194, 27, 31, 1);' class='yaowen_content1_title' href='newsDetail.html?id=" + HomeList.yaowenlist
					.data[0].id + "' >" + HomeList.yaowenlist.data[0].title + "</a> ")
				for (let index = 1; index < 5; index++) {
					$("#yaowenList").append("<div><span class='dian'>·</span><a href='newsDetail.html?id=" + HomeList.yaowenlist.data[
						index].id + "'>" + HomeList.yaowenlist.data[index].title + "</a></div>")
				}

				//第二个card
				$("#yanwenContent2T").append(
					"<a style='color: rgba(194, 27, 31, 1);' class='yaowen_content1_title' href='newsDetail.html?id=" + HomeList.yaowenlist
					.data[
						5].id + "' >" + HomeList.yaowenlist.data[5].title + "</a> ")
				for (let index = 6; index < 10; index++) {
					$("#yaowenList2").append("<div><span class='dian'>·</span><a href='newsDetail.html?id=" + HomeList.yaowenlist.data[
						index].id + "'>" + HomeList.yaowenlist.data[index].title + "</a></div>")
				}
				//第三个card
				$("#yanwenContent3T").append(
					"<a style='color: rgba(194, 27, 31, 1);' class='yaowen_content1_title' href='newsDetail.html?id=" + HomeList.yaowenlist
					.data[
						10].id + "' >" + HomeList.yaowenlist.data[10].title + "</a> ")
				for (let index = 11; index < 15; index++) {
					$("#yaowenList3").append("<div><span class='dian'>·</span><a href='newsDetail.html?id=" + HomeList.yaowenlist.data[
						index].id + "'>" + HomeList.yaowenlist.data[index].title + "</a></div>")
				}
				//第四个
				if (HomeList.yaowenlist.data[16]) {
					for (let index = 15; index < 17; index++) {
						$("#yaowen13").append("<div><img src='" + HomeList.yaowenlist.data[index].thumbnail +
							"' style='width: 240px;height: 160px;'><div class='yaowen_bottom_twotext'><a style='color: #333333;' href='newsDetail.html?id=" +
							HomeList.yaowenlist.data[index].id + "'>" + HomeList.yaowenlist.data[index].title + "</a></div></div>")
					}
				}
			...
			if (HomeList.shendulist.data != null) {
				...
				$("#shenduTitleM").attr("onclick", "leibiaoToDetail(" + HomeList.shendulist.cate_id + ")")
				$("#shenduImg").attr("src", HomeList.shendulist.data[0].thumbnail)
				$("#shenduList1T").append("<a style='color: #C21B1F;' href='newsDetail.html?id=" + HomeList.shendulist.data[0].id +
					"'>" + HomeList.shendulist.data[0].title + "</a>") //第一个标题 1Title
				for (let index = 1; index < HomeList.shendulist.data.length; index++) {
					$("#shenduList").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" + HomeList.shendulist
						.data[index].id + "'>" + HomeList.shendulist.data[index].title + "</a></div>")
				}
			}
			//访谈处理
			if (HomeList.fangtanlist.data != null) {
				...
				$("#fangtanList1T").append("<a style='color: #333333;' href='newsDetail.html?id=" + HomeList.fangtanlist.data[0]
					.id + "'>" + HomeList.fangtanlist.data[0].title + "</a>") //第一个标题 1Title

				for (let index = 1; index < HomeList.fangtanlist.data.length; index++) {
					$("#fangtanList").append(
						"<div class='fangtan_content_card d-flex'  style='margin-top: 17px;'><img src='" + HomeList.fangtanlist.data[
							index]
						.thumbnail +
						"' style='width:140px;height:93px;margin-right: 15px;'/><div><div class='fangtan_content_card_t' style='color: rgba(194, 27, 31, 1);'><a style='color: #666666;' href='newsDetail.html?id=" +
						HomeList.fangtanlist.data[index].id + "'>" +
			...
			if (HomeList.yanbianlist.data != null) {
				...
				$("#yanbianContentC7_time").append(HomeList.jiaotonglist.data[0].create_time)
				...
				for (let a = 1; a < HomeList.yanbianlist.data.length; a++) {
					$("#yanbianList7").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
						HomeList.yanbianlist.data[a].id +
			...
			if (HomeList.jchenglist.data != null) {
				...
				$("#yanbianContentC8_time").append(HomeList.jiaotonglist.data[0].create_time)
				...
				for (let a = 1; a < HomeList.jchenglist.data.length; a++) {
					$("#yanbianList8").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
						HomeList.jchenglist.data[a].id +
			...
			if (HomeList.gonganlist.data != null) {
				...
				$("#yanbianContentC9_time").append(HomeList.gonganlist.data[0].create_time)
				...
				for (let a = 1; a < HomeList.gonganlist.data.length; a++) {
					$("#yanbianList9").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
						HomeList.gonganlist.data[a].id +
						"'>" + HomeList.gonganlist.data[a].title + "</a></div>")
				}

			}
			if (HomeList.jiaotonglist.data != null) {
				console.log(HomeList.jiaotonglist.data);
				...
				$("#yanbianContentC10_time").append(HomeList.jiaotonglist.data[0].create_time)
				...
				for (let a = 1; a < HomeList.jiaotonglist.data.length; a++) {
					$("#yanbianList10").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
						HomeList.jiaotonglist.data[a].id +
			...
			//法制处理
			if (HomeList.fazhilist.data != null) {
			
				$("#fazhi").append(HomeList.fazhilist.title)
				$("#fazhiM").attr("onclick", "leibiaoToDetail(" + HomeList.fazhilist.cate_id + ")")
				
				// $("#fazhiTitle").append("<a style='color: #333333;' href='newsDetail.html?id=" + HomeList.fazhilist.data[0].id +
				// 	"'>" + HomeList.fazhilist.data[0].title + "</a>")
				
				// $("#fabuContentT3").append("<a style='color: #922530;' href='newsDetail.html?id=" + HomeList.wenyuanlist.data[3]
				// 	.id +
				// 	"'>" + HomeList.wenyuanlist.data[3].title + "</a>")
				
				// for (let i = 3; i < 8; i++) {
				// 	$("#fazhiContentList").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
				// 		HomeList.fazhilist.data[i].id +
				// 		"'>" + HomeList.fazhilist.data[i].title + "</a> </div>")
				// }
				for (let i = 0; i < HomeList.fazhilist.data.length; i++) {
					$("#fazhiContentList").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
						HomeList.fazhilist.data[i].id +
						"'>" + HomeList.fazhilist.data[i].title + "</a> </div>")
				}
			...
			//社会处理
			if (HomeList.shehuilist.data != null) {
			
				$("#shehui").append(HomeList.shehuilist.title)
				$("#shehuiM").attr("onclick", "leibiaoToDetail(" + HomeList.shehuilist.cate_id + ")")
				...
				// 		"'>" + HomeList.fazhilist.data[i].title + "</a> </div>")
				// }
				for (let i = 0; i < HomeList.shehuilist.data.length; i++) {
					$("#shehuiContentList").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
						HomeList.shehuilist.data[i].id +
						"'>" + HomeList.shehuilist.data[i].title + "</a> </div>")
				}
			}
			...
			if (HomeList.wenyuanlist.data != null) {
				...
				// $("#fabuTitle").append("<a style='color: #333333;' href='newsDetail.html?id=" + HomeList.wenyuanlist.data[0].id +
				// 	"'>" + HomeList.wenyuanlist.data[0].title + "</a>")
				for (let index = 0; index < 2; index++) {
					$("#fabutext2Cont").append("<div><img src='" + HomeList.wenyuanlist.data[index].thumbnail +
						"' style='width: 140px;height: 93px;'><div class='fabutext2'><a style='color: #333333;height:15px' href='newsDetail.html?id=" +
						HomeList.wenyuanlist.data[index].id + "'>" + HomeList.wenyuanlist.data[index].title +
						"</a></div></div>")
				}
				// $("#fabuContentT3").append("<a style='color: #922530;' href='newsDetail.html?id=" + HomeList.wenyuanlist.data[3]
				// 	.id +
				// 	"'>" + HomeList.wenyuanlist.data[3].title + "</a>")
				$("#fabuContentT4").append("<a style='color: #922530;' href='newsDetail.html?id=" + HomeList.wenyuanlist.data[7]
					.id +
					"'>" + HomeList.wenyuanlist.data[7].title + "</a>")
				for (let i = 2; i < 7; i++) {
					$("#fabuContentList").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
						HomeList.wenyuanlist.data[i].id +
						"'>" + HomeList.wenyuanlist.data[i].title + "</a> </div>")
				}
				for (let i = 8; i < 12; i++) {
					$("#fabuContentList2").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
						HomeList.wenyuanlist.data[i].id +
						"'>" + HomeList.wenyuanlist.data[i].title + "</a> </div>")
				}
			}
			//通化处理
			if (HomeList.tonghualist.data != null) {
			console.log(HomeList.tonghualist.data);
				$("#tonghua").append(HomeList.tonghualist.title)
				$("#tonghuaM").attr("onclick", "leibiaoToDetail(" + HomeList.tonghualist.cate_id + ")")
				
				$("#tonghuaTitle").append("<a style='color: #333333;' href='newsDetail.html?id=" + HomeList.tonghualist.data[0].id +
					"'>" + HomeList.tonghualist.data[0].title + "</a>")
				
				// $("#tonghuaContentT3").append("<a style='color: #922530;' href='newsDetail.html?id=" + HomeList.tonghualist.data[3]
				// 	.id +
				// 	"'>" + HomeList.tonghualist.data[3].title + "</a>")
				
				// for (let i = 3; i < 8; i++) {
				// 	$("#fazhiContentList").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
				// 		HomeList.tonghualist.data[i].id +
				// 		"'>" + HomeList.tonghualist.data[i].title + "</a> </div>")
				// }
				$("#tonghuaImg").attr("src", HomeList.tonghualist.data[0].thumbnail)
				$("#tonghuaList1T").append("<a style='color: #333333;' href='newsDetail.html?id=" + HomeList.tonghualist.data[0]
					.id + "'>" + HomeList.tonghualist.data[0].title + "</a>") //第一个标题 1Title
				for (let i = 1; i <  HomeList.tonghualist.data.length; i++) {
					
					$("#tonghuaContentList").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
						HomeList.tonghualist.data[i].id +
						"'>" + HomeList.tonghualist.data[i].title + "</a> </div>")
				}
			}
			...
			if (HomeList.yingzhanlist.data != null) {
				...
				$.each(HomeList.yingzhanlist.data, function(index, value) {
					$("#yingzhanList").append("<div onclick='yinzhanToDetail(" + HomeList.yingzhanlist.data[index].id +
						")' style=''><img src='" + HomeList.yingzhanlist.data[index].thumbnail +
						"' style='width: 140px;height: 93px;'><div class='yingzhangcard_t'>" + HomeList.yingzhanlist.data[index].title +
						"</div></div>")
				})
			}
			//人事楷模 财经观察
			if (HomeList.xwfblist.data != null) {
				...
				$("#renshiContentT13").append("<a style='color: #333333;' href='newsDetail.html?id=" + HomeList.xwfblist.data[0]
					.id + "'>" + HomeList.xwfblist.data[0].title + "</a>")
				...
				for (let a = 1; a < HomeList.xwfblist.data.length; a++) {
					$("#renshiList13").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
						HomeList.xwfblist.data[a].id +
			...
			if (HomeList.kaimolist.data != null) {
				...
				$("#renshiContentT14").append("<a style='color: #333333;' href='newsDetail.html?id=" + HomeList.kaimolist.data[0]
					.id + "'>" + HomeList.kaimolist.data[0].title + "</a>")
				...
				for (let a = 1; a < HomeList.kaimolist.data.length; a++) {
					$("#renshiList14").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
						HomeList.kaimolist.data[a].id +
			...
			if (HomeList.caijinglist.data != null) {
				...
				$("#renshiContentT15").append("<a style='color: #333333;' href='newsDetail.html?id=" + HomeList.caijinglist.data[
					0].id + "'>" + HomeList.caijinglist.data[0].title + "</a>")
				...
				for (let a = 1; a < HomeList.caijinglist.data.length; a++) {
					$("#renshiList15").append("<div><span class='dian'>·</span><a class='diva' href='newsDetail.html?id=" +
						HomeList.caijinglist.data[a].id +
			...
			if (res.data.vlist.data != null) {
				...
				$("#videoContent1").append(
					"<div style='margin-bottom:10px'><img src='" + videolist[0].thumbnail + "' onclick='yinzhanToDetail(" +
					videolist[0].id + ")' style='width:550px;height:309px' ><div class='video_title'>" + videolist[0].title +
					"</div>"
				...
				for (let index = 1; index < videolist.length; index++) {
					$("#videoContent2").append("<div style='margin-bottom:10px'><img src='" + videolist[index].thumbnail +
						"' onclick='yinzhanToDetail(" + videolist[index].id +
						")' style='width:240px;height:135px' ><div class='video_title'>" + videolist[index].title + "</div></div>")
				}
			}
			//客户端微博展示处理
	...
	$.ajax({
		type: "post", //请求方式
		url: "http://admin.jrjl.net/api/home/pc/bannerlist",
		dataType: "json",
		data: {
		...
		success: function(res) {
			...
			$.each(res.data.list, function(index) {
				$("#gg" + (index + 1) + "").attr('src', res.data.list[index].thumbnail)
				if(res.data.list[index].url){
					console.log(typeof(res.data.list[index].url) );
					
					$("#gg" + (index + 1) + "").attr("onclick", 'ggLianjie("' + res.data.list[index].url + '")')
				}
